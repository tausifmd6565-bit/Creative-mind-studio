...
const realDistribution = {
  ...
  async schedule(payload: SchedulePublicationPayload): Promise<PlatformVariant> {
    return (await apiClient.post<PlatformVariant>(DISTRIBUTION.schedule(payload.projectId, payload.variantId), payload)).data;
  },
  async publish(projectId: string, variantId: string): Promise<PlatformVariant> {
    return (await apiClient.post<PlatformVariant>(DISTRIBUTION.publish(projectId, variantId))).data;
  },
};

// ─── Exported service — no backend yet, always use mock ──────────────────────
