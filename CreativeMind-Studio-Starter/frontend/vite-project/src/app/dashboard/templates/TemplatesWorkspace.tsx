...
const TemplateCard: React.FC<{
  ...
  onUse: () => void;
  index: number;
}> = ({ template, selected, onSelect, onUse, index }) => {
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="flex items-center justify-between pt-1 border-t border-white/[0.04]">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] text-[#9D6CFF] hover:text-white transition-colors font-medium"
          >
...
const TemplateDetailPanel: React.FC<{ template: Template; onUse: () => void }> = ({ template, onUse }) => {
  const Icon = CONTENT_TYPE_ICON[template.contentType];
  const color = CONTENT_TYPE_COLOR[template.contentType];
...
export const TemplatesWorkspace: React.FC<TemplatesWorkspaceProps> = ({ onUseTemplate }) => {
        ...
        {selected && (
          <div className={`${showDetail ? 'flex w-full xl:w-[300px]' : 'hidden xl:flex xl:w-[300px]'} flex-col flex-shrink-0 border-l border-white/[0.05]`}>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <TemplateDetailPanel template={selected} onUse={() => handleUseTemplate(selected)} />
              </motion.div>
            </AnimatePresence>
          </div>
        )}
