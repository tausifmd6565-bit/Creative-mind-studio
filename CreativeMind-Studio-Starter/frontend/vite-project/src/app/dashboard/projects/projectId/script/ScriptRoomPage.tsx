...
export const ScriptRoomPage: React.FC<ScriptRoomPageProps> = ({
  ...
  // Real data support: load script generator data from pipeline result
  const [pipelineData, setPipelineData] = useState<any>(null);

  useEffect(() => {
    if (projectId === 'demo-project') return;
    import('../../../../../services/pipeline.service').then(({ pipelineService }) => {
      pipelineService.getResult(projectId)
        .then(res => {
          if (res?.story_generator) {
            setPipelineData(res.story_generator);
          }
        })
        .catch(() => {});
    });
  ...
  // Map pipeline structure beat descriptions to script blocks
  const activeBlocks = useMemo(() => {
    if (!pipelineData?.story_beats) return data.blocks;
    return pipelineData.story_beats.map((beat: any, idx: number) => ({
      id: `real-b-${idx}`,
      sectionId: idx === 0 ? 'sec-hook' : idx === 1 ? 'sec-setup' : idx === 2 ? 'sec-conflict' : idx === 3 ? 'sec-resolution' : 'sec-cta',
      type: 'narration' as const,
      speaker: 'Nova Voiceover',
      parentSceneId: `sc-${idx + 1}`,
      content: beat.description,
      durationSeconds: beat.duration_seconds || 45,
      isApproved: true,
      updatedAt: new Date(),
    }));
  }, [data.blocks, pipelineData]);

  // Adjust story sections based on narrative_arc/synthesis beat count
  const activeSections = useMemo(() => {
    if (!pipelineData?.narrative_arc) return data.sections;
    return [
      { id: 'sec-hook', title: 'The Hook', progressPercent: 100, sceneCount: 1, durationSeconds: 30, color: '#EF4444' },
      { id: 'sec-setup', title: 'Act I: Setup', progressPercent: 100, sceneCount: 2, durationSeconds: 120, color: '#3B82F6' },
      { id: 'sec-conflict', title: 'Act II: Conflict', progressPercent: 100, sceneCount: 3, durationSeconds: 240, color: '#F59E0B' },
      { id: 'sec-resolution', title: 'Act III: Resolution', progressPercent: 100, sceneCount: 1, durationSeconds: 90, color: '#10B981' },
      { id: 'sec-cta', title: 'CTA / Outro', progressPercent: 100, sceneCount: 1, durationSeconds: 20, color: '#EC4899' },
    ];
  }, [data.sections, pipelineData]);
  ...
  const leftPanel = (
    <div className="h-full flex flex-col overflow-hidden">
      <PanelHeader
        ...
        subtitle={`${activeSections.length} stages · click to filter script`}
        color="#F59E0B"
      />
