...
export const ViralityTwinPage: React.FC<ViralityTwinPageProps> = ({
  ...
  const [conceptTitle, setConceptTitle] = useState(data.conceptCard.title);
  ...
  }, [setBreadcrumbs, setPrimaryAction, setActiveNavId, conceptTitle]);

  // Run virality analysis against real backend
  const handleRunAnalysis = useCallback(async () => {
    setIsRunning(true);
    setViralityError(null);
    try {
      await viralityService.analyse(projectId, filters);
      setHasRealData(true);
      setConceptTitle(prev => prev);
    } catch (_err) {
      // Silently fall back to mock data — backend may not have a session yet
      setViralityError('Could not reach backend — showing sample data');
    } finally {
      setIsRunning(false);
    }
  }, [projectId, filters]);
  ...
  return (
    ...
    >
      ...
      {viralityError && (
        <div className="mx-4 sm:mx-6 lg:mx-8 mt-4 flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 font-mono text-[11px]">
          <span>⚠️ {viralityError}</span>
        </div>
      )}
      {hasRealData && (
        <div className="mx-4 sm:mx-6 lg:mx-8 mt-4 flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono text-[11px]">
          <span>✓ Real analysis completed with IBM Granite</span>
        </div>
      )}
