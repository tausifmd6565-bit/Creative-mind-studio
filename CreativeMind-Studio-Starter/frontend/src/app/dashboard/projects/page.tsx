...
}> = React.memo(({ project, index, onOpen }) => {
  const statusCfg = PROJECT_STATUS[project.status];
  const phaseColor = PHASE_COLORS[project.activePhase] ?? '#8B5CF6';
...
export const ProjectsPage: React.FC<ProjectsPageProps> = ({ onCreateProject, onOpenProject }) => {
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState<ProjectStatus | 'all'>('all');
  const [apiProjects, setApiProjects] = useState<ProjectCard[]>([]);
  const [loadingProjects, setLoadingProjects] = useState(false);

  // Load real projects from backend on mount
  useEffect(() => {
    setLoadingProjects(true);
    projectService.list()
      .then(res => {
        const mapped: ProjectCard[] = (res.data ?? []).map((p: { id: string; title: string; status: string; current_stage?: string }) => ({
          id: p.id,
          title: p.title,
          description: '',
          platform: 'YouTube',
          thumbnailGradient: 'from-violet-600 to-purple-900',
          status: (p.status === 'active' ? 'in-progress' : p.status) as ProjectStatus ?? 'draft',
          overallProgress: 0,
          activePhase: ((p.current_stage ?? 'strategy').toLowerCase()) as PhaseId,
          activePhaseLabel: p.current_stage ?? 'Strategy',
          activePhaseStatus: 'in-progress' as const,
          responsible: { name: 'You', initials: 'ME', color: '#8B5CF6' },
          blockedCount: 0,
          aiActive: false,
          aiAgentName: 'IBM Granite',
          decisionsLogged: 0,
          approvalsTotal: 0,
          approvalsPending: 0,
  ...
  // Show real projects strictly from backend API
  ...
  const stats = useMemo(() => ({
    ...
    blocked: allProjects.reduce((sum, p) => sum + p.blockedCount, 0),
    aiLive:  allProjects.filter(p => p.aiActive).length,
  }), [allProjects]);
