...
class Project(Base):
    ...
    research_packs: Mapped[list[ResearchPack]] = relationship(
        "ResearchPack", back_populates="project", cascade="all, delete-orphan"
    )
    blueprints: Mapped[list[CreativeBlueprint]] = relationship(
        "CreativeBlueprint", back_populates="project", cascade="all, delete-orphan"
    )
...
class ResearchPack(Base):
    ...
    questions: Mapped[list | None] = mapped_column(JSON, nullable=True)   # list of str
    facts: Mapped[list | None] = mapped_column(JSON, nullable=True)        # list of str
    sources: Mapped[list | None] = mapped_column(JSON, nullable=True)      # list of {title, url, type}
    ...
    # relationships
    project: Mapped[Project] = relationship("Project", back_populates="research_packs")
...
class CreativeBlueprint(Base):
    ...
    scenes: Mapped[list | None] = mapped_column(JSON, nullable=True)               # list of scene objects
    broll: Mapped[list | None] = mapped_column(JSON, nullable=True)                # list of suggestions
    editing_notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    platform_adaptations: Mapped[dict | None] = mapped_column(JSON, nullable=True) # {platform: notes}
    production_timeline: Mapped[list | None] = mapped_column(JSON, nullable=True)  # list of phase objects
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
